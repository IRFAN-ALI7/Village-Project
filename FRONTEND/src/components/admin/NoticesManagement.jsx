...
export default function NoticesManagement() {
  ...
  const fetchNotices = async () => {
    try {
      ...
      toast.error(
        err.response?.data?.message || "Failed to load notices"
      );
  ...
  const handleCreateNotice = async () => {
    if (
      ...
      !newNotice.category ||
      !newNotice.date ||
      !newNotice.description.trim() ||
      !newNotice.fullDetails.trim()
    ) {
    ...
    try {
      ...
      toast.success(
        res.data.message || "Notice created successfully!"
      );
      ...
      // Add newly created notice immediately to list.
      if (res.data.notice) {
        setNotices((prev) => [
          res.data.notice,
          ...prev,
  ...
  const handleEditClick = (notice) => {
    setSelectedNotice({
      ...
      fullDetails: notice.fullDetails || "",
  ...
  const handleUpdateNotice = async () => {
    if (
      ...
      !selectedNotice.fullDetails?.trim()
    ) {
      toast.error("Please fill all required fields!");
      return;
    }
    ...
    try {
      ...
      const updateData = {
        ...
        location: selectedNotice.location,
        description: selectedNotice.description,
        fullDetails: selectedNotice.fullDetails,
        isPinned: selectedNotice.isPinned,
      };

      const res = await axios.put(
        `${API_URL}/admin/notices/${selectedNotice._id}`,
        updateData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      toast.success(
        res.data.message || "Notice updated successfully!"
      );
    } catch (err) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${API_URL}/admin/notices/${deleteNoticeId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      setNotices((prev) =>
        prev.filter(
          (notice) => notice._id !== deleteNoticeId
        )
      );
    ...
    return (
      <span
        className={`px-3 py-1 rounded-full text-xs font-bold ${
          badges[category] || badges.general
        }`}
      >
        ...
        <div className="space-y-4">
          {notices.length > 0 ? (
            notices.map((notice) => (
              <div
                key={notice._id}
                className="bg-gradient-to-r from-orange-50 to-yellow-50 border-2 border-orange-200 rounded-xl p-4 md:p-6 hover:shadow-lg transition-all"
              >
                <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      ...
                      {notice.isPinned && (
                        <Pin className="h-4 w-4 text-red-600 fill-red-600 shrink-0" />
                      )}
                    </div>
            ...
            <div className="bg-gray-50 border-2 border-gray-200 rounded-xl p-10 text-center">
              ...
              <p className="text-gray-400 text-sm mt-1">
                Create a notice for your Panchayat.
              </p>
            </div>
          )}
        </div>
            ...
            <div className="p-4 md:p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 bg-gradient-to-r from-green-600 to-blue-600 text-white py-3 rounded-xl hover:shadow-lg font-semibold text-sm"
                >
                  Create Notice
                </button>
                ...
                <button
                  type="button"
                  onClick={() => setShowCreateModal(false)}
                  className="flex-1 bg-gray-200 text-gray-700 py-3 rounded-xl hover:bg-gray-300 font-semibold text-sm"
                >
                  Cancel
                </button>
              </div>
            ...
            <div className="p-4 md:p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 bg-blue-600 text-white py-3 rounded-xl hover:bg-blue-700 font-semibold text-sm"
                >
                  Update Notice
                </button>
            ...
            <div className="p-4 md:p-6">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 bg-red-600 text-white py-3 rounded-xl hover:bg-red-700 font-semibold text-sm"
                >
                  Yes, Delete
                </button>
