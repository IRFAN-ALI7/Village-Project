...
export default function ComplaintsManagement() {
  ...
  const filteredComplaints = complaints.filter((complaint) => {
    ...
    const complaintId = complaint.complaintId?.toLowerCase() || "";
  ...
  // Search/filter change par first page
  useEffect(() => {
    setCurrentPage(1);
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span
        className={`px-3 py-1 rounded-full text-xs font-bold border ${className}`}
      >
        {status
  ...
  const handleUpdateStatus = async () => {
    if (!selectedComplaint?._id) {
      toast.error("Complaint not selected.");
      return;
    }
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/admin/status-update/${selectedComplaint._id}`,
        {
          method: "PUT",
          headers: {
      ...
      // Backend response ke according updated complaint
      const updatedComplaint =
        data.complaint || data.updatedComplaint;
      ...
      toast.success(
        `Complaint ${selectedComplaint.complaintId} status updated successfully!`
      );

      setNewStatus(newStatus);
      setStatusReason(statusReason.trim());
    } catch (error) {
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-semibold">
                    {selectedComplaint.userId?.mobile ||
